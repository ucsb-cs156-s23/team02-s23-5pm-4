...
public class BookControllerTests extends ControllerTestCase {
        ...
        public void logged_in_user_can_get_all_books() throws Exception {
                ...
                expectedBooks.addAll(Arrays.asList(pjo, hg));
        ...
        public void an_admin_user_can_post_a_new_book() throws Exception {
                ...
                Book parameterized = Book.builder()
                ...
                when(bookRepository.save(eq(parameterized))).thenReturn(parameterized);

                // act
                MvcResult response = mockMvc.perform(
                                post("/api/books/post?name=Parameterized Algorithms&author=Daniel Lokshtanov&genre=Fantasy&wordcount=123456")
                                                .with(csrf()))
                                .andExpect(status().isOk()).andReturn();

                // assert
                verify(bookRepository, times(1)).save(parameterized);
                String expectedJson = mapper.writeValueAsString(parameterized);
                String responseString = response.getResponse().getContentAsString();
                assertEquals(expectedJson, responseString);
        }
        ...
        public void admin_can_delete_a_book() throws Exception {
                ...
                when(bookRepository.findById(eq(15L))).thenReturn(Optional.of(pjo));
        ...
        public void admin_can_edit_an_existing_book() throws Exception {
                ...
                Book pjo_edited = Book.builder()
                        .name("Percy Jackson and the Sea of Monsters")
                        .author("Rick Riordannn")
                        .genre("scary")
                        .wordcount(132514)
                        .build();

                String requestBody = mapper.writeValueAsString(pjo_edited);

                when(bookRepository.findById(eq(67L))).thenReturn(Optional.of(pjo));
                ...
                verify(bookRepository, times(1)).save(pjo_edited); // should be saved with updated info
                String responseString = response.getResponse().getContentAsString();
        ...
        public void admin_cannot_edit_book_that_does_not_exist() throws Exception {
                ...
                String requestBody = mapper.writeValueAsString(edited_pjo);
